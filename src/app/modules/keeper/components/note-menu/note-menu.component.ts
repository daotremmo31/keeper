...
export class NoteMenuComponent implements OnInit {
  ...
  deleteConfirmDialogue(): void {
    if (this.newNote) {
      this.deleteNote();
      this.snackBar.open("Đã chuyển ghi chú vào thùng rác", null, {
        duration: 2000,
      });
      ...
      this.deleteNote();
      this.snackBar.open("Đã khôi phục ghi chú", null, { duration: 2000 });
    }
    this.deleteNoteEmit();
  }
  ...
  arhiveClick() {
    ...
    if (this.note.arhieved)
      this.snackBar.open("Đã chuyển ghi chú vào kho lưu trữ", null, {
        duration: 2000,
      });
    else
      this.snackBar.open("Đã khôi phục ghi chú từ kho lưu trữ", null, {
        duration: 2000,
      });
  ...
  //
  showMoreMenuItem() {
    if (this.newNote && this.noteService.checkNoteIsEmpty(this.note))
      return false;
    else return true;
  }
}
